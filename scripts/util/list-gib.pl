...
my $gibfcount = 0         ; # Count of gibberish file names found.
...
sub curfile ($path) {
   ++$filecount;
   if ( get_name_from_path($path) =~ m/^$gibpat/ ) {
      ++$gibfcount;
      say $path;
   }
   return 1;
...
sub stats {
   if ( $Verbose >= 1 ) {
      ...
      say "Found $filecount files, $gibfcount of them with GIB names.";
   }
   return 1;
} # end sub stats
...
sub help {
   ...
   Welcome to "list-gib.pl". This program prints the paths of all files in the
   current directory (and all subdirectories if a -r or --recurse option is used)
   which have "gib" names (names with roots consisting of 8 random lower-case
   letters, used for randomizing file names).

   -------------------------------------------------------------------------------
   Command lines:

   list-gib.pl -h | --help                       (to print this help and exit)
   list-gib.pl [options] [Arg1] [Arg2] [Arg3]    (to list gib file names)
   ...
   Happy gibberish-name file listing!
   Cheers,
   Robbie Hatley,
   programmer.
   END_OF_HELP
