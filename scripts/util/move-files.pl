...
sub argv {
   ...
   $src = $CLArgs[0];
   $dst = $CLArgs[1];
   if ( ! -e $src    ) {die "Error: srce \"$src\" doesn't exist.    \n";}
   if ( ! -d $src    ) {die "Error: srce \"$src\" isn't a directory.\n";}
   if ( ! -e $dst    ) {die "Error: dest \"$dst\" doesn't exist.    \n";}
   if ( ! -d $dst    ) {die "Error: dest \"$dst\" isn't a directory.\n";}
