...
# Note: This version does not allow code or captures in replacements; use "substitute-dangerous.pl" for that.
#
...
#                   Got rid of string eval in this "safe" version. Now allowing all flags except "/e".
...
{ # begin main
   ...
   my $glob = $Sflg =~ m/g/ ? 'g' : ''; my $Cmnd = "s/$Regx/$Repl/$glob";
   while (<STDIN>) {
      if ($Debg) {
         warn "Command which would have been executed = \"$Cmnd\".\n";
         next;
      }
      if ('g' eq $glob) {s/$Regx/$Repl/g;} else {s/$Regx/$Repl/;}
      print;
   }
...
sub argv {
   ...
   if ( $Flgs =~ m/e/ ) {
      warn $_ for (<DATA>);
      warn "\nError: Flag \"e\" is disallowed to prevent this program\n"
          ."from being used to execute arbitrary snippets of Perl code.\n"; exit 170;
   }
...
sub help {
   ...
   Captures and code execution in replacements are disallowed; if you need those
   features, use my "substitute-dangerous.pl" program instead.

   All modifier letters are allowed except for "/e", which is disallowed to
   prevent execution of arbitrary snippets of Perl code.

   However, while not disallowed, some modifiers will do nothing, others will
   cause warnings to be printed, and some will cause this program to crash.
   For example:
   "/r" WOULD discard the substitution, but this program ignores it.
   ...
   substitute-safe.pl [-h | --help]               (to get help)
   substitute-safe.pl [options] arg1 arg2 [arg3]  (to substitute)
   ...
   Cmnd Line:                          Meaning:
   substitute-safe.pl arg1 arg2        s/arg1/arg2/
   substitute-safe.pl arg1 arg2 arg3   s/arg1/arg2/arg3
