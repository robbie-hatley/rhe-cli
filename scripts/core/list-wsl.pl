...
my $wslfcount = 0         ; # Count of wsl file names found.
...
sub curfile ($path) {
   ++$filecount;
   if ( get_name_from_path($path) =~ m/^$wslpat/ ) {
      ++$wslfcount;
      say $path;
   }
   return 1;
...
sub stats {
   if ( $Verbose >= 1 ) {
      ...
      say "Found $filecount files, $wslfcount of them with WSL names.";
   }
   return 1;
} # end sub stats
...
sub help {
   ...
   Welcome to "list-wsl.pl". This program prints the paths of all files in the
   current directory (and all subdirectories if a -r or --recurse option is used)
   which have wsl names (names with roots consisting of 64-digit hexadecimal
   numbers, assigned by Microsoft Windows 10 Spotlight).

   -------------------------------------------------------------------------------
   Command lines:

   list-wsl.pl -h | --help                       (to print this help and exit)
   list-wsl.pl [options] [Arg1] [Arg2] [Arg3]    (to list wsl file names)
