...
##############################################################################################################
# wordle-helper.pl
# Helps user solve Wordle puzzles.
...
# Word lists:
my @words    = ()         ; # Possible Wordle answers.
...
{ # begin main
   ...
   # # Die if word length is not 4, 5, 6, or 7:
   # if ( 4 != $wl && 5 != $wl && 6 != $wl && 7 != $wl ) {
   #    die "Error: Word length must be 4, 5, 6, or 7.\n";
   # }
   ...
   # Get list of all possible Wordle answers from data:
   foreach my $line (<DATA>) {chomp $line; push @words, $line;}
...
sub help {
   ...
   Welcome to "$pname". This program helps users solve Wordle puzzles.

   -------------------------------------------------------------------------------
   Command lines:

   $pname  -h | --help     (to print this help and exit)
   $pname  [options]       (to help solve Wordle puzzles)
   ...
   letter must be 's'.". The number of commas must be one-less-than the word
   length (so 4 for standard 5-letter Wordle).
   ...
   be t, but e and o and t must be present in the word". The number of commas
   must be one-less-than the word length (so 4 for standard 5-letter Wordle).
   ...
   space-free string of unique letters, such as "owpqr". If not provided, this
