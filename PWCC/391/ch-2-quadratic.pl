...
"@tails" such that $tails[$i] = "least ending value (tail) seen so far for strictly-increasing subsequences of
length $i+1". The "least tails" method as I implement it here has quadratic complexity. I could use a binary
search to reduce the complexity, but I'm not inclined to endure the hassle, and it would obfuscate the code,
so I'll stick with the quadratic variant of the "least tails" method.
...
# PRAGMAS, MODULES, AND SUBS:
   ...
   # (Length Of Longest Strictly-Increasing Subsequence - "least tails" method - quadratic complexity)
   sub LOLSIS ( @sequence ) {
      ...
      # for SISs of length $i+1.
      for my $number (@sequence) {
         my $i;
         for ( $i = 1 ; $i <= $#waggly_tails ; ++$i) {
            last if $number <= $waggly_tails[$i]; # new longest SIS if last fails for all $i
         }
