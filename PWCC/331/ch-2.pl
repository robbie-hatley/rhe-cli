...
Input: $source = "fram"
       $target = "farm"
Output: true
Swapping 'a' and 'r' makes it buddy strings.
...
3. If the number of indices for which the characters are unequal is not 2, the strings aren't buddies.
3. If the second pair of unequal characters is not the reverse of the first, the strings aren't buddies.
4. Otherwise, the strings are buddies.
...
./ch-2.pl '(["granite", "gabro"], ["granite", "gnarite"], ["dog", "dog"], ["rook", "rook"])'
...
# PRAGMAS, MODULES, AND SUBS:
   ...
   sub buddies ($s1, $s2) {
      ...
      # Else if the two strings are different, the number of differences must be 2,
      # and the second character pair must be the reverse of the first character pair:
      else {
         my @ue = (); # ue = UnEqual character pairs
         for my $idx (0..$n1-1) {
